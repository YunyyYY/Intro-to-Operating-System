        ...
        if (strcmp(child_argv[0], "cd") == 0) {  // ------- [cd] -------
            ...
            for (int i = 0; i < num_path; i++) {
                ...
                if (ret == 0) {
                    ...
                    if (pip_flag)  //printf("pipe\n");
                    {
                        pip_prog = 0;  // potential pipe program
                        for (int j = 0; j < num_path; j++) {
                            strcpy(prog2, "\0");
                            strcat(prog2, path[j]);
                            strcat(prog2, sec_argv[0]);
                            ret = access(prog2, X_OK);
                            if (ret == 0) {
                                pip_prog = 1;   // sucess pipe program flag
                                break;  // pipe program name: prog2
                            }
                        }
                        if (pip_prog==0)
                            break;
                        ...
                        int pid2 = fork();
                        if (pid2 == 0) {
                            int dp2 = dup2(pipe_fd[0],0);
                            close(pipe_fd[0]);
                            if (dp2 < 0) {
                                write(STDERR_FILENO, error_message, strlen(error_message));
                                exit(0);
                            }
                            if (execv(prog2, sec_argv) == -1){
                                write(STDERR_FILENO, error_message, strlen(error_message));
                                exit(0);
                            }
                        }
                        ...
                        pid_t rc = fork();   // create child process
            ...
            } //printf("311: ret = %d, pip_prog = %d", ret, pip_prog);
            if ((ret != 0) | (pip_prog == 0))
                write(STDERR_FILENO, error_message, strlen(error_message));
        }
    }
