...
// Fetch the pstat at addr from process p.
int
fetchstat(struct proc *p, uint addr, struct pstat **pstatp)
{
  if(addr >= p->sz || addr+4 > p->sz)
    return -1;
...
int
argstat(int n, struct pstat **pstatp)
{
  return fetchstat(proc, proc->tf->esp + 4 + 4*n, pstatp);
}
